...
// to refresh the cell-link list at every call to force subroutine.
{
	...
	cout << "inside neighbor lists rlstsq value is "<< rlstsq<<"\n";
	for (icell = 1; icell <= ncell; icell++) {
		...
		while (i >= 0) {
			...
			while (j >= 0) {
				...
				{
					//cout << "the same atom "<<i<<" is twice in the root cell\n";
				}


				if(rijsq<=0.8*0.8)
				{
					cout<<rijsq<<"WARNING *** ATOMS TOO CLOSE \t";
					cout << i<<" "<<atom[i].elem<<" "<<j<<" "<<atom[j].elem<<"\t";
					cout << atom[i].rx << " "<<atom[i].ry<<" "<<atom[i].rz<<"\t";
					cout << atom[j].rx<<" "<<atom[j].ry <<" "<< atom[j].rz<<"\n";
			...
			for (nabor = 1; nabor <= 13; nabor++) {
				...
				while (j >= 0) {
					...
					{
						//cout << "the same atom "<<i<<" is twice in the adjacent cells\n";
					}
					j = list[j];
				}
			}
