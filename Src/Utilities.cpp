...
int RunCommand(const char *strCommand, bool vfork_form)
{
  int iForkId, iStatus;
  if(vfork_form)
    {
...
int execute_command_2( char *command)
{
  ...
  iStatus = RunCommand(command,true);
  if (!iStatus)
    iNumProc++;
...
void zip (char *filename)
{
   ...
   if(UTILITIES_ZIP)
   {
    ...
    /*
	 if(system(command)==-1)
	 {
		 cout << "gzip failed.. does god know what is happeneing ? \n";
		 cout << command <<"\n";
	 }
	 */
   }
...
void unzip (char *filename)
{
   if(UTILITIES_ZIP)
   {
    ...
    /*
	 if(system(command)==-1)
	 {
		 cout << "gunzip failed.. file is expected to be as is.. after post processing it will be zipped\n";
		 return_val = -1;
	 }
	 */
   }
...
void H_crystal(double H[3][3],double crystal[6])
{
	...
	cout << "Crystal values are \t \t "<< crystal[0]<<"\t"<<crystal[1]<<"\t"<<crystal[2]<<"\t";
	cout << crystal[3]*180/PI<<"\t"<<crystal[4]*180/PI<<"\t"<<crystal[5]*180/PI<<"\n";
