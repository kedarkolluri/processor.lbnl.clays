	  ...
	  for (jnab = jbeg; jnab <= jend; jnab++)
		{
			...
			if (rijsq <= rcoordsq[atom[i].type-1][atom[j].type-1])
			{
				...
				if((i==6)||(j==6))
				{
					cout <<i<<"\t"<<j<<"\t"<<atom[i].elem<<"\t"<<atom[j].elem<<"\t"<<sqrt(rijsq)<<"\n";
				}
...
void fill_bonds_etc()
{
	...
	for(auto it = element_bonds.begin(); it !=element_bonds.end(); it++)
	{
		cout << (*it)[0]<< "\t"<<(*it)[1]<<"\t"<<(*it)[2]<<"\n";
	}
	cout <<"\nlist of available angles:";
	for(auto it = element_angles.begin(); it !=element_angles.end(); it++)
	{
