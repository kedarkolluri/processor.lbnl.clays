...
*/
	...
	for(int tt = 1;tt < argc;tt++)
	{
		if (strcmp(argv[tt], "convert_VESTA")==0)
		{
			...
			if(tt+2< argc)
			{
				...
				{
					cout << "to convert, please give `filename` followed by name of the file\n";
	...
	if(CONVERT_VESTA)
	{
		...
		delete_duplicates();
		cout << "after delete, the number of atoms are "<<n<<"\n";
		...
		// //change few of Ge atoms to Al randomly
		// auto K_to_be_kept = change_atom_types(8, 4, 0.35);

		//change few of Ge atoms to Al
		//but min distance between 2 Al atoms is more than 4Angstorms
		auto K_to_be_kept = change_atom_types_randomly(8, 4, 0.35, 3.5);
		cout << total_K-K_to_be_kept<<" dfdf\n";
		cout << "obtained ratio is \t"<< K_to_be_kept*1.0/(2*total_K)<<"\n";
		// change the remaining atoms to Si
		change_atom_types_randomly(8, 2, 1.0);
		delete_atoms_randomly(7, total_K-K_to_be_kept, 4.0);
		prepare_nbrlist(Hcry, 100);

		coord_number(Hcry);
		//calling above method is necessary for below method to work
		change_bridging_oxygen(4, 5, 10);
