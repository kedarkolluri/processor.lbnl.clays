      ...
      if(ptr_tmp_line[0]=="element_bonds")
      {
        ...
        }else
        {
          for (int i =1;i <num_entries;i=i+3)
          {
            ...
            int input_val;
            input_val = atoi(ptr_tmp_line[i].c_str());
            element_bonds.push_back(input_val);
            input_val = atoi(ptr_tmp_line[i+1].c_str());
            element_bonds.push_back(input_val);
            input_val = atoi(ptr_tmp_line[i+2].c_str());
            element_bonds.push_back(input_val);
            for(auto k=element_bonds.begin(); k!=element_bonds.end(); k=k+3)
            {
              cout << *k <<"\t"<<*(k+1)<<"\t"<<*(k+2)<<" the value of the element is \n";
            }
          }
          for(auto k=element_bonds.begin(); k!=element_bonds.end(); k=k+3)
            cout << *k <<"\t"<<*(k+1)<<"\t"<<*(k+2)<<" the value of the element is \n";
      ...
      if(ptr_tmp_line[0]=="element_angles")
      {
        ...
        }else
        {
          for (int i =1;i <num_entries;i=i+4)
          {
            ...
            element_bonds.push_back(atoi(ptr_tmp_line[i+3].c_str()));
            for(auto k=element_bonds.begin(); k!=element_bonds.end(); k=k+4)
            {
              cout << *k <<"\t"<<*(k+1)<<"\t"<<*(k+2)<<"\t"<<*(k+3)<<" the value of the element is \n";
            }
          }
          for(auto k=element_bonds.begin(); k!=element_bonds.end(); k=k+4)
            cout << *k <<"\t"<<*(k+1)<<"\t"<<*(k+2)<<*(k+3)<<" the value of the element is \n";
