...
*/
	...
	{
		angles_exist = true;
		fprintf(fptr, "%d %s\n", angles.size()/4, " angles");
	}
	fprintf(fptr, "\n");

	fprintf(fptr, "%d %s\n", n_types, "atom types");
	if(bonds_exist) fprintf(fptr, "%d %s\n\n", element_bonds.size()/3, "bond types");
	if(angles_exist) fprintf(fptr, "%d %s\n\n", element_angles.size()/4, "angle types");
	fprintf(fptr, "\n");
	...
	if(bonds_exist)
	{
		fprintf(fptr, "%s\n\n", "Bonds");
		int counter = 1;
		for(auto it = bonds.begin(); it != bonds.end(); it=it+3)
		{
			
			fprintf(fptr, "%d %d %d %d\n", counter, *it, *(it+1)+1, *(it+2)+1);
			counter++;
		}
	}
	...
	if(angles_exist)
	{
		fprintf(fptr, "%s\n\n", "Angles");
		int counter = 1;
		for(auto it = angles.begin(); it != angles.end(); it=it+4)
		{
			fprintf(fptr, "%d %d %d %d %d\n", counter, *it, *(it+1)+1, *(it+2)+1,*(it+3)+1);
			counter++;
		}
	}
