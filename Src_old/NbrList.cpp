	...
	for (iz = 1; iz <= mz; iz++)
		for (iy = 1; iy <= my; iy++)
			for (ix = 1; ix <= mx; ix++)
			{
				...
				map[imap+ 1] = cellno(ix+1,iy  ,iz  );
				map[imap+ 2] = cellno(ix+1,iy+1,iz  );
				map[imap+ 3] = cellno(ix  ,iy+1,iz  );
				map[imap+ 4] = cellno(ix-1,iy+1,iz  );
				map[imap+ 5] = cellno(ix+1,iy  ,iz-1);
				map[imap+ 6] = cellno(ix+1,iy+1,iz-1);
				map[imap+ 7] = cellno(ix  ,iy+1,iz-1);
				map[imap+ 8] = cellno(ix-1,iy+1,iz-1);
				map[imap+ 9] = cellno(ix+1,iy  ,iz+1);
				map[imap+10] = cellno(ix+1,iy+1,iz+1);
				map[imap+11] = cellno(ix  ,iy+1,iz+1);
				map[imap+12] = cellno(ix-1,iy+1,iz+1);
				map[imap+13] = cellno(ix  ,iy  ,iz+1);
...
// to refresh the cell-link list at every call to force subroutine.
{
	...
	for (i = 0; i < n; i++)
	{
		...
		if(icell<0) cout << i<<"\t"<<atom[i].rx<<"\t"<<atom[i].ry<<"\t"<<atom[i].rz<<"\n";
		if(icell>ncell)
			{
	...
	for (icell = 1; icell <= ncell; icell++) {
		...
		while (i >= 0) {
			...
			while (j >= 0) {
				...
				if(rijsq<=0.05)
				{
					cout << i<<"\t"<<atom[i].type<<"\t"<<atom[i].sz<<"\t"<<j<<"\t"<<atom[j].type<<"\t"<<atom[j].sz<<"\t"<<rijsq<<" examples of too close atoms \n";
						//exit(1);
				}
